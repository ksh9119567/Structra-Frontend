...
// Bounds mirror ORG_ACTION_POLICIES in the backend (core/constants/org_constant.py).
// invite_member / create_team / create_project range MANAGER–ADMIN; update_member
// and remove_member are fixed at ADMIN (floor == ceiling), so they aren't
// editable and are shown read-only for transparency instead.

type EditableField = "invite_member_min_role" | "create_team_min_role" | "create_project_min_role";

const EDITABLE_ROWS: { key: EditableField; label: string }[] = [
  { key: "invite_member_min_role", label: "Invite members" },
  { key: "create_team_min_role", label: "Create teams" },
  { key: "create_project_min_role", label: "Create projects" },
];

const LOCKED_ROWS: { key: "update_member_min_role" | "remove_member_min_role"; label: string }[] = [
  { key: "update_member_min_role", label: "Update members" },
  { key: "remove_member_min_role", label: "Remove members" },
];

const ROLE_OPTIONS: OrgRole[] = ["MANAGER", "ADMIN"];

// ─── Types ────────────────────────────────────────────────────────────────────

type Editable = Record<EditableField, OrgRole>;
...
type RolePermissionsEditModalProps = {
  open: boolean;
  orgId: string;
...
function toEditable(s: OrgSettings): Editable {
  return {
    ...
    create_team_min_role: s.create_team_min_role,
    create_project_min_role: s.create_project_min_role,
  };
}

// ─── Component ────────────────────────────────────────────────────────────────

export function RolePermissionsEditModal({
  open,
  orgId,
  settings,
  onClose,
  ...
  function setRole(key: EditableField, value: OrgRole) {
            ...
            {EDITABLE_ROWS.map((row) => (
              <div key={row.key} className="flex items-center justify-between gap-3">
                ...
                <select
                  id={row.key}
                  value={draft[row.key]}
                  onChange={(e) => setRole(row.key, e.target.value as OrgRole)}
                  disabled={isSaving}
          ...
          <div className="border-t border-border/60 pt-4 space-y-2">
            ...
            {LOCKED_ROWS.map((row) => (
              <div key={row.key} className="flex items-center justify-between gap-3 opacity-70">
                ...
                <OrgRoleBadge role={settings[row.key]} />
              </div>
            ))}
          </div>
        </div>
