...
/**
 * BFF: List projects belonging to an organization.
 * GET /api/organizations/[id]/projects?search=&status=&ordering=&page=&page_size=
 * Proxies to DRF GET /api/v1/projects/get_org-projects/?org_id=<id>
 */
export async function GET(request: NextRequest, { params }: RouteContext) {
  ...
  try {
    const result = await serverApi.get<PaginatedResponse<ProjectSummary>>(
      `/projects/get_org-projects/?${backendParams.toString()}`,
      { token: accessToken },
    );
