...
/**
 * BFF: Send a team invite.
 * POST /api/teams/[id]/invite
 * Body: { email: string; role?: string }
 * Proxies to DRF POST /api/v1/teams/sent-invite/
 * Body to DRF: { team_id, email, role? }
 */
export async function POST(request: NextRequest, { params }: RouteContext) {
  ...
  try {
    const result = await serverApi.post<{ message: string; invite_token: string }>(
      "/teams/sent-invite/",
      { token: accessToken, json: { team_id: id, email, role } },
    );
    ...
    if (error instanceof ApiError) {
      ...
      if (msg.includes("already") && msg.includes("member")) {
        return NextResponse.json(
          { message: "This person is already a member of the team.", field: "email" },
          { status: 400 },
        );
      }
      if (msg.includes("already") && msg.includes("invite")) {
        return NextResponse.json(
          { message: "An invite has already been sent to this email.", field: "email" },
          { status: 400 },
        );
      }
