...
const VALID_ROLES = ["MANAGER", "LEAD", "CONTRIBUTOR", "VIEWER"];

/**
 * BFF: Update a member's role in a project.
 * PUT /api/projects/[id]/members/[memberEmail]
 * Body: { role: string }
 * Proxies to DRF PUT /api/v1/projects/update-member/?project_id=<id>
...
export async function PUT(request: NextRequest, { params }: RouteContext) {
  ...
  try {
    const result = await serverApi.put<{ message: string; data: unknown }>(
      `/projects/update-member/?project_id=${id}`,
...
/**
 * BFF: Remove a member from a project.
 * DELETE /api/projects/[id]/members/[memberEmail]
 * Proxies to DRF DELETE /api/v1/projects/remove-member/?project_id=<id> body: { email }
 */
export async function DELETE(_req: NextRequest, { params }: RouteContext) {
  ...
  try {
    await serverApi.delete(`/projects/remove-member/?project_id=${id}`, {
      token: accessToken,
      json: { email },
    });
